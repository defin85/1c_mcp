...
## Локальная адаптация для VD

В этой рабочей копии добавлен контейнер `mcp_ИнструментОтладкаВД`.

Стартовые инструменты:
- `run_select_query` — выполняет только запросы 1С на чтение, начинающиеся с `ВЫБРАТЬ`, и возвращает первые строки результата в JSON.
- `analyze_receipt_payment_accounts` — возвращает шапку документа `ПоступлениеНаРасчетныйСчет`, строки `РасшифровкаПлатежа` и ключевые сведения по связанным договорам.

Этот контейнер предназначен для живой диагностики проблем в целевой ИБ `VD` без правок ядра `mcp_APIBackend`.
