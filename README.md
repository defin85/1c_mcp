...
- **Зачем он нужен?** Прокси решает ключевые инфраструктурные проблемы:
    ...
    - **Debug-only fallback:** Может скрывать или публиковать `debug_execute_bsl` через env-флаг `MCP_ALLOW_DEBUG_EXECUTE`, чтобы generic executor был доступен только на тестовых контурах.
...
### Debug-only generic executor

Для коротких разовых runtime-проверок можно использовать инструмент `debug_execute_bsl`.

- Он выполняет серверный BSL-фрагмент и ожидает, что итог будет присвоен в переменную `Результат`.
- Внутри фрагмента доступна переменная `Параметры`, которая заполняется из `parametersJson`.
- Инструмент скрыт по умолчанию и появляется только если в Python-прокси включен `MCP_ALLOW_DEBUG_EXECUTE=true`.
- Включать его стоит только на BF/TP/локальных тестовых базах. Для регулярных проверок лучше всё равно выносить логику в отдельный tool или тест.
